...
public class HarmonyPatches : MonoBehaviour
{
    ...
    class Patch1_2
    {
        static void Postfix(StageTumbler __instance)
        {
            ...
            {
                MeshRenderer[] meshes =
                    __instance.gameObject.GetComponentsInChildren<MeshRenderer>();
    ...
    class Patch2_3
    {
        static void Postfix(ref UIPartActionVariantSelector __instance)
        {
            ...
            {
                __instance.buttonPrevious.GetComponent<Image>().color =
    ...
    {
        static void Postfix(
            ref LinearControlGauges __instance,
            ref bool precisionMode,
            ref List<Image> ___inputGaugeImages
        )
        {
            if (!__instance)
                return;
            if (
                gaugeNeedleYawPitchRollColor_replace == false
                && gaugeNeedleYawPitchRollPrecisionColor_replace == false
            )
                return;
            try
            {
            ...
            foreach (var instruction in instructions)
            {
                if (instruction.Calls(getColorRed))
                {
                    instruction.operand = AccessTools.Method(
                        ...
                        nameof(HarmonyPatches.METDisplayRedColorOverride)
                    );
                }
                else if (instruction.Calls(getColorYellow))
                {
                    instruction.operand = AccessTools.Method(
                        typeof(HarmonyPatches),
                        nameof(HarmonyPatches.METDisplayYellowColorOverride)
                    );
                }
                else if (instruction.Calls(getColorGreen))
                {
                    instruction.operand = AccessTools.Method(
                        typeof(HarmonyPatches),
                        nameof(HarmonyPatches.METDisplayGreenColorOverride)
                    );
                }
    ...
    {
        static void Postfix(
            ref StageGroup __instance,
            ref TextMeshProUGUI ___DeltaVHeadingText,
            ref TextMeshProUGUI ___uiStageIndex
        )
        {
            if (__instance)
            {
    ...
    class Patch13
    {
        static void Postfix(ref VerticalSpeedGauge __instance)
        {
            ...
            {
                __instance.gauge.pointer.gameObject.GetComponentInChildren<Image>().color =
                    VerticalSpeedGaugeNeedleColor;
            }
        }
    }
    ...
    {
        static void Postfix(
            ref TextMeshProUGUI ___apoapsisAltitude,
            ref TextMeshProUGUI ___apoapsisTime,
            ref TextMeshProUGUI ___periapsisAltitude,
            ref TextMeshProUGUI ___periapsisTime,
            ref TextMeshProUGUI ___orbitPeriod
        )
        {
    ...
    {
        static void Postfix(
            ref TextMeshProUGUI ___orbitArgumentOfPeriapsis,
            ref TextMeshProUGUI ___orbitLongitudeOfAscendingNode,
            ref TextMeshProUGUI ___ejectionAngle,
            ref TextMeshProUGUI ___orbitEccentricity,
            ref TextMeshProUGUI ___orbitInclination
        )
        {
            if (ManeuverNodeEditorTextColor_replace)
            {
    ...
    {
        static void Postfix(
            ref TMP_InputField ___proRetrogradeField,
            ref TMP_InputField ___normalField,
            ref TMP_InputField ___radialField,
            ref TMP_InputField ___timeField
        )
        {
            if (ManeuverNodeEditorTextColor_replace)
            {
